...
* Адреса вкладок изменились: вместо якорей `#recrawler_ai` теперь `admin.php?page=recrawler&tab=ai`. Старые ссылки открывают первую вкладку
* Настройки журнала переехали из опции `recrawler_logs` в `recrawler_general`. Данные переносятся автоматически при первом заходе в админку
* `Logger::log()` объявляет возвращаемый тип `void` вслед за psr/log 3.x
...
* Настройки уведомлений выделены секцией «Notify», настройки журнала — секцией «Logs» внутри «Общего»
...
* WPOSA научилась секциям внутри вкладки
...
* Исправлен порядок шагов сборки в CI
